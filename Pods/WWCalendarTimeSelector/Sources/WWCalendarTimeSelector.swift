...
open class WWCalendarTimeSelector: UIViewController, UITableViewDelegate, UITableViewDataSource, WWCalendarRowProtocol, WWClockProtocol {
    ...
    open override func viewDidLoad() {
       ...
       // NotificationCenter.default.addObserver(self, selector: #selector(WWCalendarTimeSelector.didRotateOrNot), name: NSNotification.Name.UIDevice.orientationDidChangeNotification, object: nil)
...
internal class WWClock: UIView {
    ...
    internal override func draw(_ rect: CGRect) {
        ...
        if showingHour {
            ...
            let templateSize = NSAttributedString(string: "60", attributes: textAttr as [NSAttributedString.Key : Any]).size()
            let templateSizeHighlight = NSAttributedString(string: "60", attributes: textAttrHighlight as [NSAttributedString.Key : Any]).size()
            let maxSize = max(templateSize.width, templateSize.height)
            let maxSizeHighlight = max(templateSizeHighlight.width, templateSizeHighlight.height)
            ...
            for (index, element) in minutes.enumerated() {
                ...
                if element == currentMinute {
                    ...
                    if minuteStep.rawValue < 5 {
                        if element % 5 == 0 {
                            let min = NSAttributedString(string: "\(element)", attributes: textAttrHighlight as [NSAttributedString.Key : Any])
                            ctx?.saveGState()
                            ctx?.scaleBy(x: -1, y: 1)
                            ctx?.translateBy(x: radiusHighlight * cos(angle), y: -(radiusHighlight * sin(angle)))
                            ctx?.scaleBy(x: -1, y: 1)
                    ...
                    if minuteStep.rawValue < 5 {
                        if element % 5 == 0 {
                            let min = NSAttributedString(string: "\(element)", attributes: textAttr as [NSAttributedString.Key : Any])
                            ctx?.saveGState()
                            ctx?.scaleBy(x: -1, y: 1)
                            ctx?.translateBy(x: radius * cos(angle), y: -(radius * sin(angle)))
                            ctx?.scaleBy(x: -1, y: 1)
